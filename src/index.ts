...
export default class APIpeline {
  ...
  public async fetch(
    ...
    if (!serviceDefinition) {
      throw new Error(
        `Cannot fetch data from unregistered service '${service}'`
      );
    }
    ...
    const {fullPath, withoutQueryParams} = this._constructPath(
      serviceDefinition,
      options
    );
    ...
    try {
      ...
      let requestId;
      if (shouldUseCache) {
        requestId = this._buildRequestId(
          serviceDefinition,
          fullPath,
          fetchHeaders,
          fetchOptions,
      ...
      if (!res.success) {
        if (cachedData.success && cachedData.data) {
          this._log(
            `Using stale cache for ${fullPath} (network request failed)`
          );
          return cachedData.data;
        } else {
          throw new Error(
            `Cannot fetch data for ${service} online, no cache either.`
          );
        }
      }

      let parsedData = {...parsedResponseData};
    ...
    try {
      ...
      if (!service) {
        ...
        if (!this._APIServices[service]) {
          throw new Error(
            `Cannot clear cache for unregistered service : '${service}'`
          );
        }
      ...
      await Promise.all(
        keysToRemove.map((key: string) => this._APICacheDriver.removeItem(key))
      );
      return;
    } catch (err) {
      throw new Error(err);
  ...
  private _createHTTPMethods() {
    HTTP_METHODS.forEach((method: IHTTPMethods) => {
      this[method.toLocaleLowerCase()] = async (...args: any[]) =>
        this.fetch(args[0], args[1], method);
    });
  }
  ...
  private async _fetch(url: string, options?: any): Promise<IFetchResponse> {
    try {
      return {
        success: true,
        data: await this._APIOptions.fetchMethod(url, options)
      };
    } catch (err) {
      console.warn(err);
  ...
  private async _cache(
    ...
    try {
      ...
      await this._APICacheDriver.setItem(
        this._getCacheObjectKey(requestId),
      ...
      if (shouldCap) {
        const capLimit =
          serviceDefinition.capLimit || this._APIOptions.capLimit;
        const serviceDictionaryKey = this._getServiceDictionaryKey(service);
        let dictionary = await this._APICacheDriver.getItem(
          serviceDictionaryKey
        );
        if (dictionary) {
  ...
  private async _getCachedData(
    ...
    let serviceDictionary = await this._APICacheDriver.getItem(
      this._getServiceDictionaryKey(service)
    );
    ...
    if (expiration) {
      ...
      try {
        const rawCachedData = await this._APICacheDriver.getItem(
          this._getCacheObjectKey(requestId)
        );
   ...
   * @memberof APIpeline
   */
  private async _addKeyToServiceDictionary(
    service: string,
    requestId: string,
    expiration: number
  ): Promise<boolean> {
  ...
  private async _getAllKeysForService(service: string): Promise<string[]> {
    try {
      ...
      if (dictionary) {
        dictionary = JSON.parse(dictionary);
        const dictionaryKeys = Object.keys(dictionary).map(
          (key: string) => `${this._APIOptions.cachePrefix}:${key}`
        );
  ...
  private async _applyMiddlewares(
    ...
    if (middlewares && middlewares.length) {
      try {
        middlewares = middlewares.map((middleware: APIMiddleware) =>
          middleware(serviceDefinition, paths, options)
        );
        const resolvedMiddlewares = await Promise.all(middlewares);
        return _merge(...resolvedMiddlewares);
      } catch (err) {
        throw new Error(
          `Error while applying middlewares for ${serviceDefinition.path} : ${err}`
        );
      }
    } else {
      return {};
  ...
  private _constructPath(
    ...
    const {fullyParsed, withoutQueryParams} = this._parsePath(
      serviceDefinition,
      options
    );
   ...
   * @memberof APIpeline
   */
  private _mergeServicesWithDefaultValues(
    services: IAPIServices
  ): IAPIServices {
    return _mapValues(services, (service: IAPIService, serviceName: string) => {
      if (
        service.domain &&
        typeof this._APIOptions.domains[service.domain] === 'undefined'
      ) {
        throw new Error(
      ...
      if (
        service.prefix &&
        typeof this._APIOptions.prefixes[service.prefix] === 'undefined'
      ) {
        throw new Error(
